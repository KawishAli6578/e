...
export default function NavGroup({
  ...
  const { menubar: menuMaster } = useUIContext();
  // const { user } = useAuth();
  const user = null;
  const [impersonateHistoryStored] = useLocalStorage(
    LOCAL_STORAGE_CONSTANTS.IMPERSONATE_HISTORY,
    []
  );
  ...
  const navCollapse = item.children
    ?.filter((menu) => {
      return impersonateHistoryStored?.length > 0 ? menu.id !== "logout" : true;
    })
    .map((menuItem, index) => {
      if (!menuItem.display) {
        return <Fragment key={menuItem.id}></Fragment>;
      }
  ...
  const shouldDisplay = Array.isArray(item.display)
    ? item.display.some((r) => user.roles.includes(r))
    : item.display;
  if (!shouldDisplay) {
